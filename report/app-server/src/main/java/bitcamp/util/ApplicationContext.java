...
public class ApplicationContext {
  ...
  private void createBeans(String basePackage) throws Exception {
    ...
    //        // Fully-Qualified class name 을 만든 다음에
    ...
    // ==> 스트림 프로그래밍 방식
    // Set<Class<?>> classes = dirReader.lines() // 오리지널 문자 단위 스트림을 줄 단위 문자열로 나열한 스트림으로 변환한다.
    // .filter(new Predicate<String>() { // 스트림 데이터에서 걸러내는 역할을 할 필터를 꼽는다.
    // @Override
    // public boolean test(String filename) {
    // // 이 필터는 파일의 확장자가 .class 로 끝나는 경우에만 처리한다.
    // return filename.endsWith(".class");
    // }
    // }).map(new Function<String, Class<?>>() { // 파일 이름을 받아서 Class 정보를 리턴하는 플러그인 장착
    // @Override
