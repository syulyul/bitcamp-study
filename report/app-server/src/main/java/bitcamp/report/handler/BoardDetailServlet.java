  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (board == null) {
      ...
      for (AttachedFile file : board.getAttachedFiles()) {
        out.printf(
            "<a href='https://kr.object.ncloudstorage.com/bitcamp-nc7-bucket-25/board/%s'>%1$s</a>"
                + " [<a href='/board/file/delete?category=%d&no=%d'>삭제</a>]" + "<br>\n",
            file.getFilePath(), category, file.getNo());
        // out.printf("<img src='/upload/board/%s' style='height:70px'><br>\n", file.getFilePath());
      }
      ...
      out.printf("<a href='/board/delete?category=%d&no=%d'>삭제</a>\n", board.getCategory(),
          board.getNo());
      out.printf("<a href='/board/list?category=%d'>목록</a>", board.getCategory());
      out.println("</div>");
      out.println("</form>");
