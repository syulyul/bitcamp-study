...
public class MySQLBoardDao implements BoardDao {
  ...
  /*
   * select b.board_no, b.title, b.view_count, b.created_date, m.member_no, m.name from report_board
   * b inner join report_member m on b.writer=m.member_no where category=1 order by board_no desc
   */
  @Override
  public List<Board> findAll() {
    try (PreparedStatement stmt = ds.getConnection(false)
        .prepareStatement("select" + "  b.board_no, " + "  b.title, " + "  b.view_count, "
            ...
            + "  report_board b inner join report_member m on b.writer=m.member_no " + " where "
  ...
  public Board findBy(int no) {
    try (PreparedStatement stmt = ds.getConnection(false)
        .prepareStatement("select" + "  b.board_no, " + "  b.title, " + "  b.content, "
            + "  b.view_count, " + "  b.created_date, " + "  m.member_no, " + "  m.name "
            + " from  " + "  report_board b inner join report_member m on b.writer=m.member_no "
      ...
      try (ResultSet rs = stmt.executeQuery()) {
        ...
        if (rs.next()) {
          ...
          stmt.executeUpdate(
              "update report_board set" + " view_count=view_count + 1" + " where board_no=" + no);
  ...
  public int update(Board board) {
    try (PreparedStatement stmt = ds.getConnection(false).prepareStatement("update report_board set"
  ...
  public int delete(Board board) {
    try (PreparedStatement stmt = ds.getConnection(false).prepareStatement(
        "delete from report_board" + " where category=? and board_no=? and writer=?")) {
