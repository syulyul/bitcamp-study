...
public class ServerApp {
  ...
  public static final String REPORT_SESSION_ID = "report_session_id";
  ...
  private NettyOutbound processRequest(HttpServerRequest request, HttpServerResponse response) {
    ...
    try {
      ...
      List<Cookie> cookies = request2.allCookies().get(REPORT_SESSION_ID);
      if (cookies != null) {
        // 세션 ID가 있으면 이 값을 가지고 클라이언트를 구분한다.
        sessionId = cookies.get(0).value();
      } else {
        // 세션 ID가 없으면 이 클라이언트를 구분하기 위해 새 세션 ID를 발급한다.
        sessionId = UUID.randomUUID().toString();
        firstVisit = true;
      }

      // 세션 ID로 클라이언트에게 배정된 HttpSession 객체를 찾는다.
      ...
      if (firstVisit) {
        // 세션 ID가 없는 클라이언트를 위해 새로 발급한 세션ID를 쿠키로 보낸다.
        // 웹 브라우저는 이 값을 내부 메모리에 저장할 것이다.
        response.addCookie(new DefaultCookie(REPORT_SESSION_ID, sessionId));
      }
