...
import bitcamp.report.service.DefaultBoardService;
import bitcamp.report.service.DefaultMemberService;
...
@PropertySource({"classpath:bitcamp/report/config/ncloud/jdbc.properties"})
@MapperScan("bitcamp.report.dao") // Mybatis가 자동으로 생성할 DAO 객체의 인터페이스 패키지 지정
public class AppConfig {
  ...
  public SqlSessionFactory SqlSessionFactory(DataSource dataSource, ApplicationContext appCtx) throws Exception {
    ...
    // Mybatis Log4j 2.x 버전을 사용하도록 활성화시킨다.
    // 활성화시키지 않으면 Mybatis에서 로그를 출력하지 않는다.
    org.apache.ibatis.logging.LogFactory.useLog4J2Logging();

    SqlSessionFactoryBean factoryBean = new SqlSessionFactoryBean();
    factoryBean.setDataSource(dataSource);
    factoryBean.setTypeAliasesPackage("bitcamp.report.vo");
    factoryBean.setMapperLocations(appCtx.getResources("classpath:bitcamp/report/dao/mysql/*Dao.xml"));
